...
final CollectionReference requestCollection =
    Firestore.instance.collection('request_list');

class FirestoreService {
  Future<Requests> createRequest(String requestName, String requestDesc,
      String requestDate, String userID) async {
    final TransactionHandler newTransaction = (Transaction tx) async {
      final DocumentSnapshot snapshot =
          await tx.get(requestCollection.document());

      final Requests request =
          Requests(requestName, requestDesc, requestDate, userID);
  ...
  Future<void> removeRequest(String requestID) {
    return requestCollection.document(requestID).delete();
  }
}
